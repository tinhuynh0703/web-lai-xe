...
export default function TuitionProfilesPage() {
  ...
  const { data: courses = [], isLoading: isLoadingCourses } = useCoursesByDateRange();
  ...
  const { data: tuitionProfiles = [], isLoading: isLoadingProfiles } = useTuitionProfiles(
    selectedCourseId,
    maHangGplx,
  );
  ...
  useEffect(() => {
    ...
    methods.setValue("profileReceiveDate", formatToDateLocal(selectedCourse.ngay_nhan_hs));
  ...
  const tableData = useMemo(
    () =>
      (tuitionProfiles || []).map((item) => {
        const tongDaNop =
          item.lich_su_nop_hoc_phis?.reduce((sum, p) => sum + (p.so_tien_nop || 0), 0) || 0;
        return {
          maDK: item.ma_dk,
          ...
          tongDaNop,
          conLai: Math.max((item.hoc_phi || 0) - tongDaNop, 0),
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "tongDaNop",
        header: "Đã nộp",
        cell: ({ row }) => formatCurrency(row.original.tongDaNop),
      },
      {
        accessorKey: "conLai",
        header: "Còn lại",
        cell: ({ row }) => formatCurrency(row.original.conLai),
      },
      {
    ...
    <div className="min-h-[calc(100vh-64px)] bg-gray-50">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-4 space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Chọn khóa học</h2>
          <Form methods={methods} onSubmit={() => {}}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Input name="gplxClass" label="Hạng GPLX" disabled placeholder="Chọn khóa học để hiển thị" />
              ...
              <Input name="trainingClass" label="Hạng ĐT" disabled placeholder="Chọn khóa học để hiển thị" />
              <DatePicker name="profileReceiveDate" label="Ngày nhận HS" type="date" disabled />
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
            <h2 className="text-xl font-semibold text-gray-900">
              Danh sách hồ sơ học phí {selectedCourseId ? `- ${selectedCourseId}` : ""}
            </h2>
            <span className="text-sm text-gray-600">Tổng số bản ghi: {tableData.length}</span>
          </div>

          {!selectedCourseId ? (
            <p className="text-sm text-gray-500">Vui lòng chọn khóa học để hiển thị danh sách.</p>
          ) : isLoadingProfiles ? (
            <Loading />
          ) : (
            <Table
              ...
              enablePagination
              enableSorting
              onRowClick={(row) => navigate(`/ke-toan/ho-so-hoc-phi/${row.maDK}`, { state: row })}
            />
          )}
        </div>
      </div>
